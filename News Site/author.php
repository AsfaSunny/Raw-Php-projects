<?php 
    ...
    $Author_getID = $_GET['authorid'];

    $pagination_query = "SELECT * FROM `user` JOIN `post` ON user.user_id = post.author WHERE post.author = '$Author_getID'";
    $result_2 = mysqli_query($db_connect, $pagination_query) or die("Query Failed");
    $Only_AuthorRow = mysqli_fetch_assoc($result_2);
?>
        ...
        <div class="row">
            <div class="col-md-8">
               ...
               <div class="post-container">
                  <h2 class="page-heading"><?php echo $Only_AuthorRow['username']; ?></h2>
                    ...
                    <?php
                        ...
                        $query = "SELECT * FROM `post` 
                          ...
                          WHERE post.author = '$Author_getID'
...
//we made the code comment and paste it in the top, working same ways but for out web title we have to do the job this way:
                    ...
                    if (mysqli_num_rows($result_2) > 0) {
                        ...
                        if ($page > 1) {
                          echo "<li><a href='author.php?authorid={$Author_getID}&page=".($page - 1)."'>Prev</a></li>";
                        }
                        ...
                        for ($p = 1; $p <= $total_Page; $p++) { 
                          ...
                          echo "<li class='{$active}'><a href='author.php?authorid={$Author_getID}&page={$p}'>{$p}</a></li>";
                        }

                        if ($page < $total_Page) {
                          echo "<li><a href='author.php?authorid={$Author_getID}&page=".($page + 1)."'>Next</a></li>";
                        }
                        echo "</ul>";
                      }
                    ?>
